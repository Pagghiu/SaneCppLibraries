...
- Wake pooled peers only after the whole range is visible so they can steal from the opposite deque end.
...
steal from the opposite end, and drain exactly once. External validation, capacity failure, cancellation, and reuse
tests remain mandatory. Recursive fan-out benchmarks must compare the local transaction against scalar publication.
