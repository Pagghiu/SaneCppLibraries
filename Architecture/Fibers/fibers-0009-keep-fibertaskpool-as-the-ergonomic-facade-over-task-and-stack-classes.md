...
stack acquisition fails, and returns both slots only after the completed fiber has returned to its worker root context.
Pool capacity and `waitForSpawnCapacity()` describe the ability to attempt one coupled spawn; they do not reserve a
future slot.
...
acquisition returns the task slot, completed pooled fibers release only after switching to the worker root context, and
the facade does not hide allocation or lifetime ownership.
