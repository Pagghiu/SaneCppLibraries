...
- A valid growth fault must lie in the next bounded uncommitted interval and the interrupted stack pointer must lie in
  ...
  forwarded to the previously installed platform handler. The POSIX action permits same-signal re-entry so the
  re-entrancy guard can forward a fault raised inside growth handling rather than deferring and retrying it.
