---
name: sane-adoption-guide
description: Guide third-party AI agents through adopting Sane C++ Libraries in an existing project. Use when deciding between single-file libraries and full-repo integration, planning the smallest useful library set, wiring SC.cpp or SANE_CPP_IMPLEMENTATION, checking platform link requirements, or explaining SC_COMPILER_ENABLE_STD_CPP and plugin export requirements to external users.
---
...
- Route deeper API usage to the relevant `sane-<library>` skill when that skill exists.
- Route example hunting to `$sane-example-finder`.
