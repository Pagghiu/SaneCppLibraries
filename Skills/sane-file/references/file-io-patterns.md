...
- Use `sane-file-system` for path and directory mutation.
- Use `sane-process` when the descriptor is part of a launched child process.
- Use `sane-async` or `sane-async-streams` for event-loop-driven descriptor handling.
