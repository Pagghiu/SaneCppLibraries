...
- Use `file` when you need descriptor-level piping or inheritance around the serial connection.
- Use `process` if the serial workflow is only one piece of a broader device-tool command flow.
