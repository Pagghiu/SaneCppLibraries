...
## Use `sane-file-system-watcher` For
...
- Seed the watcher from paths discovered with `sane-file-system-iterator`.
- Use the async template when you already have an event loop.
- Route hot-reload state changes through `sane-plugin-build-recipes` when the watcher drives plugin reloads.

## Hand Off To Other Skills

- Use `sane-async` for event-loop integration.
- Use `sane-file-system` for the mutation that happens after a watched event.
- Use `sane-plugin-build-recipes` for reload-oriented compositions.
