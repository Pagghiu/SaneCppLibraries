...
- Use file descriptors or pipes from `sane-file` for redirection endpoints.

## Hand Off To Other Skills

- Use `sane-file` for pipe descriptors and inheritance.
- Use `sane-async` when child I/O is part of an event loop.
- Use `sane-file-system` for preparing the working directory or input paths.
