...
{
    String path;
    SC_TEST_EXPECT(not Path::relativeFromTo(path, "/a", "", Path::AsPosix));
    SC_TEST_EXPECT(not Path::relativeFromTo(path, "", "/a", Path::AsPosix));
    SC_TEST_EXPECT(not Path::relativeFromTo(path, "", "", Path::AsPosix));
    SC_TEST_EXPECT(Path::relativeFromTo(path, "/", "/a/b/c//", Path::AsPosix) and path == "a/b/c");
    SC_TEST_EXPECT(Path::relativeFromTo(path, "/a/b/_1/2/3", "/a/b/_d/e", Path::AsPosix, Path::AsPosix) and
                   path == "../../../_d/e");
    SC_TEST_EXPECT(Path::relativeFromTo(path, "C:\\a\\b", "C:\\a\\c", Path::AsWindows, Path::AsWindows) and
                   path == "..\\c");
    SC_TEST_EXPECT(not Path::relativeFromTo(path, "/a", "b/c", Path::AsPosix));
    SC_TEST_EXPECT(not Path::relativeFromTo(path, "a", "/b/c", Path::AsPosix));
    SC_TEST_EXPECT(Path::relativeFromTo(path, "/a/b", "/a/b", Path::AsPosix) and path == ".");
    SC_TEST_EXPECT(Path::relativeFromTo(path, "/a/b/c/d/e/f/g/h", "/a/b/c/d/e", Path::AsPosix, Path::AsPosix) and
                   path == "../../..");
}
