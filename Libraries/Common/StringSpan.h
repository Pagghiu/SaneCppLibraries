...
struct SC_FOUNDATION_EXPORT StringSpan
{
    // clang-format off
    ...
    [[nodiscard]] Comparison compare(StringSpan other) const
    {
        if (getEncoding() == other.getEncoding())
        {
            ...
            const int cmp = CompilerBuiltins::compare(text, other.text, minSize);
            if (cmp != 0)
        ...
        while (p1 < end1 and p2 < end2)
        {
            ...
            uint32_t cp2 =
                other.getEncoding() == StringEncoding::Utf16 ? advanceUTF16(p2, end2) : advanceUTF8(p2, end2);
            if (cp1 < cp2)
...
#endif // SC_FOUNDATION_STRING_SPAN_DEFINITION_H
