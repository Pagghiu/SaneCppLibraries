...
struct FiberStackClassInternal
{
    ...
    Result reserve(const FiberStackClassOptions& options)
    {
        ...
        if (options.commitMode == FiberStackCommitMode::Incremental)
        {
            if (not FiberStackGrowthRuntime::isSupported())
            {
                resetMetadata();
        ...
        const size_t committedStackOffset = freeStackOffset + options.maxStacks * sizeof(size_t);
        metadataBytes =
            FiberVirtualMemory::roundUpToPageSize(committedStackOffset + options.maxStacks * sizeof(size_t));
    ...
    Result acquire(FiberStack& outStack)
    {
        ...
        Span<char> stackMemory = slotMemory(index);
        Result     commitResult =
            fiberCommitReadWrite(stackMemory.data() + stackBytes - initialCommitBytes, initialCommitBytes);
        if (not commitResult)
        {
        ...
        {
            committedMemory(index).fillHighWaterMark();
        }
        fiberSchedulerUnlock(availabilityQueue.lock);
        return Result(true);
    }
    ...
    void diagnostics(FiberStackClassDiagnostics& outDiagnostics) const
    {
        ...
        outDiagnostics.growthCommitSizeInBytes  = growthCommitBytes;
        fiberSchedulerUnlock(availabilityQueue.lock);
    }
    ...
    void resetMetadata()
    {
        ...
        peakHighWaterBytes  = 0;
        initialCommitBytes  = 0;
        growthCommitBytes   = 0;
        commitMode          = FiberStackCommitMode::Full;
...
static thread_local FiberJobWorker* currentFiberJobWorker = nullptr;
...
static FiberJobWorker* currentJobWorkerFor(FiberJobScheduler& scheduler)
{
    return currentFiberJobWorker != nullptr and currentFiberJobWorker->isActive() and
                   currentFiberJobWorker->scheduler() == &scheduler
               ? currentFiberJobWorker
               : nullptr;
}
...
FiberJobWorker::FiberJobWorker() = default;

FiberJobWorker::~FiberJobWorker()
{
    SC_FIBERS_ASSERT_RELEASE(not workerActive);
    SC_FIBERS_ASSERT_RELEASE(workerScheduler == nullptr);
    SC_FIBERS_ASSERT_RELEASE(localQueueScheduler == nullptr);
    SC_FIBERS_ASSERT_RELEASE(workerJob == nullptr);
    SC_FIBERS_ASSERT_RELEASE(localDeque == nullptr);
    SC_FIBERS_ASSERT_RELEASE(localDequeAllocator == nullptr);
    SC_FIBERS_ASSERT_RELEASE(localDequeCapacity == 0);
    SC_FIBERS_ASSERT_RELEASE(fiberAtomicLoadSize(localDequeTop) == 0);
...
bool FiberJobWorker::isActive() const { return workerActive; }

FiberJobScheduler* FiberJobWorker::scheduler() { return workerScheduler; }

const FiberJobScheduler* FiberJobWorker::scheduler() const { return workerScheduler; }

FiberJob* FiberJobWorker::runningJob() { return workerJob; }

const FiberJob* FiberJobWorker::runningJob() const { return workerJob; }

Result FiberJobWorker::begin(FiberJobScheduler& scheduler)
{
...
void FiberJobWorker::end()
{
    SC_FIBERS_ASSERT_RELEASE(currentFiberJobWorker == this);
    SC_FIBERS_ASSERT_RELEASE(workerJob == nullptr);
    currentFiberJobWorker = nullptr;
    workerActive          = false;
    workerScheduler       = nullptr;
}
...
Result FiberJobScheduler::create(Span<FiberJob*> readyStorage)
{
    ...
    queueTail    = 0;
    queueCount   = 0;
    fiberAtomicStoreSize(readyJobs, 0);
    fiberAtomicStoreSize(activeJobs, 0);
    fiberAtomicStoreUInt32(cancelGeneration, 0);
    runningJob = nullptr;
                                             ...
                                             size_t capacityPerWorker)
{
    ...
    for (FiberJobWorker& worker : workers)
    {
        ...
        void*        memory     = allocator.allocate(&worker, dequeBytes, alignof(FiberJob*));
        if (memory == nullptr)
        {
            releaseWorkerDeques({workers.data(), allocatedWorkers});
            for (size_t index = 0; index < allocatedWorkers; ++index)
        ...
        fiberAtomicStoreSize(worker.ownedReadyJobs, 0);
...
void FiberJobScheduler::releaseWorkerDeques(Span<FiberJobWorker> workers)
{
    for (FiberJobWorker& worker : workers)
    {
        SC_FIBERS_ASSERT_RELEASE(not worker.workerActive);
        SC_FIBERS_ASSERT_RELEASE(worker.workerScheduler == nullptr);
        ...
        SC_FIBERS_ASSERT_RELEASE(fiberAtomicLoadSize(worker.ownedReadyJobs) == 0);
...
bool FiberJobScheduler::tryPushWorkerDeque(FiberJobWorker& worker, FiberJob& job)
{
...
FiberJob* FiberJobScheduler::popWorkerReady(FiberJobWorker& worker)
{
    ...
    FiberJob* job = worker.localDeque[bottom % worker.localDequeCapacity];
    ...
    if (job != nullptr)
    {
        volatile size_t& readyCounter = usesDistributedAccounting(worker) ? worker.ownedReadyJobs : readyJobs;
        SC_FIBERS_ASSERT_RELEASE(fiberAtomicLoadSize(readyCounter) > 0);
        fiberAtomicFetchSubSize(readyCounter, 1);
    }
    return job;
}

FiberJob* FiberJobScheduler::stealWorkerReady(FiberJobWorker& victim)
{
    size_t top = fiberAtomicLoadSize(victim.localDequeTop);
    if (top >= fiberAtomicLoadSize(victim.localDequeBottom))
    {
        return nullptr;
    }

    FiberJob* job = victim.localDeque[top % victim.localDequeCapacity];
    SC_FIBERS_ASSERT_RELEASE(job != nullptr);
    if (not fiberAtomicCompareExchangeSize(victim.localDequeTop, top, top + 1))
    {
        return nullptr;
    }
    volatile size_t& readyCounter = usesDistributedAccounting(victim) ? victim.ownedReadyJobs : readyJobs;
    SC_FIBERS_ASSERT_RELEASE(fiberAtomicLoadSize(readyCounter) > 0);
    fiberAtomicFetchSubSize(readyCounter, 1);
    return job;
}

FiberJob* FiberJobScheduler::stealReady(FiberJobWorker& worker, Span<FiberJobWorker> workerGroup)
{
    if (workerGroup.sizeInElements() <= 1)
    {
        return nullptr;
    }

    worker.stealAttempts += 1;
    const size_t start = worker.stealCursor % workerGroup.sizeInElements();
    for (size_t offset = 0; offset < workerGroup.sizeInElements(); ++offset)
    {
        const size_t    victimIndex = (start + offset) % workerGroup.sizeInElements();
        FiberJobWorker& victim      = workerGroup[victimIndex];
        if (&victim == &worker or victim.localDeque == nullptr or victim.localQueueScheduler != this)
        {
            continue;
        }
        FiberJob* job = stealWorkerReady(victim);
        if (job != nullptr)
        {
            worker.stealCursor = (victimIndex + 1) % workerGroup.sizeInElements();
            worker.stolenJobs += 1;
            return job;
        }
    }
    worker.failedSteals += 1;
    worker.stealCursor = (start + 1) % workerGroup.sizeInElements();
    return nullptr;
}
...
Result FiberJobScheduler::runOne(FiberJobWorker& worker, Span<FiberJobWorker> workerGroup, bool& outRanJob)
{
    ...
    if (job == nullptr)
    {
        job = stealReady(worker, workerGroup);
    }
    if (job == nullptr)
    {
        worker.end();
...
Result FiberJobScheduler::run(FiberJobWorker& worker, Span<FiberJobWorker> workerGroup)
{
    while (hasReadyJobs())
    {
        bool ranJob = false;
...
{
    fiberAtomicFetchAddUInt32(cancelGeneration, 1);
    return Result(true);
}

bool FiberJobScheduler::isOpen() const { return queueStorage.sizeInElements() != 0; }

bool FiberJobScheduler::hasReadyJobs() const { return readyJobCount() != 0; }
...
size_t FiberJobScheduler::readyJobCount() const
{
    size_t ready = fiberAtomicLoadSize(readyJobs);
...
{
    FiberJobWorker* worker = currentJobWorkerFor(*this);
    return worker != nullptr ? worker->workerJob : runningJob;
}
