...
int SC::AsyncEventLoop::getTotalNumberOfActiveHandle() const
{
    return numberOfActiveHandles + numberOfTimers + numberOfWakeups + numberOfExternals;
}

SC::Result SC::AsyncEventLoop::runOnce() { return runStep(PollMode::ForcedForwardProgress); }

SC::Result SC::AsyncEventLoop::runNoWait() { return runStep(PollMode::NoWait); }

void SC::AsyncEventLoop::completeAndEventuallyReactivate(KernelQueue& queue, AsyncRequest& async, Result&& returnCode)
{
...
SC::Result SC::AsyncEventLoop::runStep(PollMode pollMode)
{
    KernelQueue queue;
    SC_LOG_MESSAGE("---------------\n");
...
void SC::AsyncEventLoop::completeAsync(KernelQueue& queue, AsyncRequest& async, Result&& returnCode, bool& reactivate)
{
    if (returnCode)
    {
...
SC::Result SC::AsyncEventLoop::stopAsync(AsyncRequest& async)
{
    SC_LOG_MESSAGE("{} {} STOP\n", async.debugName, AsyncRequest::TypeToString(async.type));
